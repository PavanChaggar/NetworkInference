...
class NetworkDiffusion(Model):
    ...
    def f(self, p, t, theta):
        ...
        Returns:
            du: values of the ode model evalulate at parameters p, theta.
        """
        k = theta
        du = k * (-self.L() @ p)
        return du
    ...
    def forward(self, u0): 
        ...
        Args:
            u0 (array): initial values for protein concentration vector and 
            model parameters

        Returns:
            u: numerical solution to forward model
        """
        p = np.exp(u0[:-1])
        theta = u0[-1]
